<template>
  <q-page class="hero-page q-pa-md participantes-page">
    ...
    <div class="row q-col-gutter-md q-mb-md">
      <div v-for="metric in metrics" :key="metric.label" class="col-12 col-sm-6 col-md-3">
    ...
    <q-dialog v-model="emsDialogOpen">
      <q-card class="form-dialog">
        <q-card-section>
          <div class="text-h6">{{ editingEmsId ? 'Editar EMS' : 'Nuevo EMS' }}</div>
        </q-card-section>
        <q-card-section>
          <q-form class="row q-col-gutter-md" @submit.prevent="saveEms">
            <q-input v-model="emsForm.nombres" class="col-12 col-md-6" label="Nombres" :rules="[requiredRule]" dense />
            <q-input v-model="emsForm.apellidoPaterno" class="col-12 col-md-6" label="Apellido paterno" :rules="[requiredRule]" dense />
            <q-input v-model="emsForm.apellidoMaterno" class="col-12 col-md-6" label="Apellido materno" dense />
            <q-input v-model="emsForm.correo" class="col-12 col-md-6" label="Correo" type="email" :rules="[requiredRule]" dense />
            <q-input v-model="emsForm.institucion" class="col-12 col-md-6" label="Institución" :rules="[requiredRule]" dense />
            <q-input v-model="emsForm.carrera" class="col-12 col-md-6" label="Carrera" :rules="[requiredRule]" dense />
            <q-input v-model="emsForm.telefono" class="col-12 col-md-6" label="Teléfono" mask="###############" :rules="[requiredRule]" dense />
    ...
    <q-dialog v-model="utvmDialogOpen">
      <q-card class="form-dialog">
        <q-card-section>
          <div class="text-h6">{{ editingUtvmId ? 'Editar UTVM' : 'Nuevo UTVM' }}</div>
        </q-card-section>
        <q-card-section>
          <q-form class="row q-col-gutter-md" @submit.prevent="saveUtvm">
            <q-input v-model="utvmForm.nombres" class="col-12 col-md-6" label="Nombres" :rules="[requiredRule]" dense />
            <q-input v-model="utvmForm.apellidoPaterno" class="col-12 col-md-6" label="Apellido paterno" :rules="[requiredRule]" dense />
            <q-input v-model="utvmForm.apellidoMaterno" class="col-12 col-md-6" label="Apellido materno" dense />
            <q-input v-model="utvmForm.correo" class="col-12 col-md-6" label="Correo" type="email" :rules="[requiredRule]" dense />
            <q-input v-model.number="utvmForm.cuatrimestre" class="col-12 col-md-4" label="Cuatrimestre" type="number" min="1" max="11" :rules="[requiredRule]" dense />
            <q-input v-model="utvmForm.grupo" class="col-12 col-md-4" label="Grupo" :rules="[requiredRule]" dense />
            <q-input v-model="utvmForm.telefono" class="col-12 col-md-4" label="Teléfono" mask="###############" :rules="[requiredRule]" dense />
            <div class="col-12 row justify-end q-gutter-sm">
              <q-btn flat label="Cancelar" v-close-popup />
              <q-btn color="primary" label="Guardar" type="submit" :loading="saving" />
            </div>
    ...
    <q-dialog v-model="externoDialogOpen">
      <q-card class="form-dialog">
        <q-card-section>
          <div class="text-h6">{{ editingExternoId ? 'Editar externo' : 'Nuevo externo' }}</div>
        </q-card-section>
        <q-card-section>
          <q-form class="row q-col-gutter-md" @submit.prevent="saveExterno">
            <q-input v-model="externoForm.nombre" class="col-12 col-md-6" label="Nombre" :rules="[requiredRule]" dense />
            <q-input v-model="externoForm.apellidoPaterno" class="col-12 col-md-6" label="Apellido paterno" :rules="[requiredRule]" dense />
            <q-input v-model="externoForm.apellidoMaterno" class="col-12 col-md-6" label="Apellido materno" dense />
            <q-input v-model="externoForm.correo" class="col-12 col-md-6" label="Correo" type="email" :rules="[requiredRule]" dense />
            <q-input v-model="externoForm.telefono" class="col-12 col-md-6" label="Teléfono" mask="###############" :rules="[requiredRule]" dense />
            <q-input v-model="externoForm.institucion" class="col-12 col-md-6" label="Institución" dense />
            <q-select v-model="externoForm.dias" class="col-12 col-md-6" :options="diasOptions" label="Días" multiple emit-value map-options dense />
            <q-input v-model.number="externoForm.total" class="col-12 col-md-3" label="Total" type="number" min="0" dense />
            <q-select v-model="externoForm.status" class="col-12 col-md-3" :options="externoStatusOptions" label="Estatus" emit-value map-options dense />
            <q-file v-if="!editingExternoId" v-model="externoComprobante" class="col-12" label="Comprobante" accept=".pdf,.jpg,.jpeg,.png" dense />
            <div class="col-12 row justify-end q-gutter-sm">
              <q-btn flat label="Cancelar" v-close-popup />
              <q-btn color="primary" label="Guardar" type="submit" :loading="saving" />
            </div>
...
const emsColumns: QTableColumn[] = [
  ...
  { name: 'apellidoPaterno', label: 'Apellido paterno', field: 'apellidoPaterno', align: 'left', sortable: true },
...
const utvmColumns: QTableColumn[] = [
  ...
  { name: 'apellidoPaterno', label: 'Apellido paterno', field: 'apellidoPaterno', align: 'left', sortable: true },
  { name: 'correo', label: 'Correo', field: 'correo', align: 'left' },
  { name: 'cuatrimestre', label: 'Cuatrimestre', field: 'cuatrimestre', align: 'center', sortable: true },
...
const nsuColumns: QTableColumn[] = [
  { name: 'created_at', label: 'Fecha', field: (row: RegistroNsu) => formatDate(row.created_at), align: 'left', sortable: true },
  { name: 'total_participantes', label: 'Participantes', field: 'total_participantes', align: 'center', sortable: true },
...
const externosColumns: QTableColumn[] = [
  { name: 'nombre', label: 'Nombre', field: (row: ParticipanteExterno) => fullExternoName(row), align: 'left', sortable: true },
  { name: 'correo', label: 'Correo', field: 'correo', align: 'left' },
  { name: 'telefono', label: 'Teléfono', field: 'telefono', align: 'left' },
  { name: 'dias', label: 'Días', field: (row: ParticipanteExterno) => row.dias.join(', '), align: 'left' },
  { name: 'total', label: 'Total', field: (row: ParticipanteExterno) => money(row.total), align: 'right' },
...
const metrics = computed(() => {
  const nsuValidaciones = nsu.value.map((registro) => mapNsuStatus(registro.estado_pago));
  const externoValidaciones = externos.value.map((externo) => mapExternoStatus(externo.status));
  const validaciones = [...nsuValidaciones, ...externoValidaciones];

  return [
    { label: 'Participantes directos', value: ems.value.length + utvm.value.length },
    { label: 'Registros con voucher', value: nsu.value.length + externos.value.length },
    { label: 'Pendientes', value: validaciones.filter((status) => status === 'pendiente').length },
    { label: 'Validados', value: validaciones.filter((status) => status === 'validado').length },
  ];
});

const requiredRule = (value: unknown) => Boolean(value) || 'Este campo es obligatorio';
...
function fullExternoName(externo: ParticipanteExterno) {
  return [externo.nombre, externo.apellidoPaterno, externo.apellidoMaterno].filter(Boolean).join(' ');
}
...
function mapExternoStatus(status: ParticipanteExterno['status']): ParticipanteEstatus {
  if (status === 'validado') return 'validado';
  if (status === 'rechazado') return 'rechazado';
  return 'pendiente';
}
...
async function deleteEms(id: number) {
  if (!window.confirm('¿Eliminar participante EMS?')) return;
...
async function deleteUtvm(id: number) {
  if (!window.confirm('¿Eliminar participante UTVM?')) return;
...
async function saveExterno() {
  ...
  try {
    if (editingExternoId.value) {
      ...
      await api.post('externos', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
    }
...
async function setNsuStatus(id: string, status: RegistroNsu['estado_pago']) {
  await removeWithReload(() => updateNsuStatus(id, status), 'Estatus NSU actualizado.');
}


async function deleteNsu(id: string) {
  if (!window.confirm('¿Eliminar registro NSU?')) return;
  await removeWithReload(() => removeNsu(id), 'Registro NSU eliminado.');
}

async function setExternoStatus(id: string, status: ParticipanteExterno['status']) {
  await removeWithReload(() => updateExternoStatus(id, status), 'Estatus de externo actualizado.');
}
...
async function removeWithReload(action: () => Promise<unknown>, successMessage: string) {
